...
ciam_default_mcs = @defsim begin
    ...
    slrcost.movefactor  = Truncated(Normal(1,.5),0.5,3)       # From Diaz (2016); incorporates communication with Mendelsohn and Anthoff and Tol (2014)
    slrcost.dvbm        = Truncated(Normal(5.376,1.344),0.0,Inf) # Updated to 2010USD from FUND
    slrcost.vslel       = Truncated(Normal(0.47,0.075),0.0,Inf) # From Viscusi and Aldy (2003)
    slrcost.vslmult     = Truncated(Normal(200,50),0.0,Inf) # From FUND (originally Cline (1992))
    slrcost.wvel        = Truncated(Normal(1.16, 0.23),0.0,Inf) # From Brander et al (2006)
    slrcost.wvpdl       = Truncated(Normal(0.47,0.06),0.0,1.0) # From Brander et al (2006)
end
