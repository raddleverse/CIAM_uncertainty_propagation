...
# set directories
rdir <- "/Users/aewsma/codes/CIAM_adaptation_regimes/R-code"
ciamdir <- "/Users/aewsma/.julia/dev/MimiCIAM"
gisdir <- "/Users/aewsma/codes/CIAM-Work/code/CIAM-GIS"

setwd(rdir)
source("proc_Diva_pop_data.R")

# Read CIAM/DIVA data 
xsc <- read_csv(paste0(ciamdir,"/data/input/xsc.csv"))
isl <- xsc$seg[xsc$island==1]
diva_db <- read_csv(paste0(ciamdir,"/data/input/data.csv")) %>% group_by(X1) %>%
  mutate(area=sum(area1,area2,area3,area4,area5,area6,area7,area8,area9,area10,area11,area12,area13,area14,area15))
diva_latlon <- read_csv(paste0(ciamdir,"/data/diva_segment_latlon.csv"))

diva_lines <- read_sf(paste0(gisdir,"/COASTLINES/diva_published-master-data-gis/data/gis/cls.shp"))
...
# Read Area Data
area_data <- raster(paste0(gisdir,"/GPW-LANDAREA-30/gpw-v4-land-water-area-rev11_landareakm_30_sec_tif/gpw_v4_land_water_area_rev11_landareakm_30_sec.tif"))

# Read Merkens Data 
ssp_2010m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2010.tif"))
ssp_2020m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2020.tif"))
ssp_2030m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2030.tif"))
ssp_2040m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2040.tif"))
ssp_2050m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2050.tif"))
ssp_2060m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2060.tif"))
ssp_2070m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2070.tif"))
ssp_2080m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2080.tif"))
ssp_2090m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2090.tif"))
ssp_2100m <- raster(paste0(gisdir,"/POPDATA/MERKENS_2016/DIVA_SSP",SSP,"_2100.tif"))

# Jones Data: Read coastal population data as raster. Unit: # of persons  
## ncname commented out - not actually used anywhere
...
ssp_2010j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2010.nc4"),varname="Band1")
ssp_2020j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2020.nc4"),varname="Band1")
ssp_2030j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2030.nc4"),varname="Band1")
ssp_2040j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2040.nc4"),varname="Band1")
ssp_2050j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2050.nc4"),varname="Band1")
ssp_2060j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2060.nc4"),varname="Band1")
ssp_2070j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2070.nc4"),varname="Band1")
ssp_2080j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2080.nc4"),varname="Band1")
ssp_2090j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2090.nc4"),varname="Band1")
ssp_2100j <- raster(paste0(gisdir,"/POPDATA/popdynamics-1-km-downscaled-pop-base-year-projection-ssp-2000-2100-rev01-proj-ssp",SSP,"-netcdf/SSP",SSP,"_1km/ssp",SSP,"_total_2100.nc4"),varname="Band1")


# Read coastal buffer data 
buffer <- read_sf(paste0(gisdir,"/buffer/buffer2.shp"))
...
###########################################
## for 2010-2100 altogether (TW version) ##
###########################################

years <- c(2010,2020,2030,2040,2050,2060,2070,2080,2090,2100)
outname <- paste0(gisdir,"/output_ssp",SSP,"_",min(years),"_",max(years),".csv")
...
for (i in ciam_Names) {
  for (y in years) {
    tmp <- outdf %>% filter(year==y,ciamID==i)
    if (nrow(tmp)>1){
      dupes <- c(dupes,i)
    }
  }
...
for (i in dupes){
  ...
  try( {seg2 <- st_polygonize(seg)
    ...
    pvars_list <- vector("list", length(years))
    for (y in 1:length(years)) {
      pvars_list[[y]] <- vector("list", 2); names(pvars_list[[y]]) <- c("m","j") # Merkens and Jones
      for (d in names(pvars_list[[y]])) {
        ssp_info <- get(paste0("ssp_",years[y],d))
        pvars_list[[y]][[d]] <- crop(ssp_info, seg2)
        pvars_list[[y]][[d]] <- mask(pvars_list[[y]][[d]], seg2)
      }
    }
    # pvars_list[[y]][[d]] is for year y and data set d. so pvars_list[[1]][["m"]] = p_m10, eg.
    # and note that pvars is both pvars (Merkens) and jvars (Jones)

    for (y in 1:length(years)){
      year <- years[y]
      pvar <- get(pvars_list[[y]]$m)
      jvar <- get(pvars_list[[y]]$j)
...
outdf_new0 <- outdf %>% filter(!(ciamID %in% dupes))
outdf_new0 <- rbind(outdf_new0,outdf2)

psegs <- ciam_Names[!(ciam_Names%in%unique(outdf_new0$ciamID))]
prob_segs <- c(psegs)

outdf_new1 <- outdf_new0 %>% filter(!(ciamID %in% prob_segs))
...
outdf_new2 <- outdf %>% filter(!(ciamID %in% iss_segs))
outdf_new2 <- rbind(outdf_new2,tmp)

# Try again with problem segments
prob_segs <- c(ciam_Names[!(ciam_Names %in% outdf_new2$ciamID)],"UnitedKingdom8057")%>% unique()
...
buffer_prob <- read_sf(paste0(gisdir,"/divaProbs/buffer_probs3.shp"))
buffer_prob <- buffer_prob %>% filter(ciamID %in%prob_nonpolys)
for (j in prob_nonpolys){
  tmp3 <- compute_seg_v3(j,years,tmp3)
}

outdf_fin <- outdf_new2 %>% filter(!(ciamID %in% unique(tmp3$ciamID)))
...
pvars_listm <- pop_listm <- vector("list", length(years))
for (y in 1:length(years)) {
  ssp_info <- get(paste0("ssp_",years[y],"m"))
  pvars_listm[[y]] <- crop(ssp_info, seg2)
  pvars_listm[[y]] <- mask(pvars_listm[[y]], seg2)
  pop_listm[[y]] <- sum(pvars_listm[[y]]@data@values, na.rm=T)
} # pvars_listm[[y]] = pYYm; poplistm[[y]] = popYY


for (yr in years){
  tmp1 <- data.frame(segID=seg2@data$segID, year=yr, areaKm2=areaval,
                     pop_jones=0, pop_merkens=0, 
                     popdens_jones=0, popdens_merkens=0, 
...
if(FALSE) {
#############################################################
## OLD (CL) VERSION WITH 2010-2050 SEPARATE FROM 2060-2100 ##
#############################################################
