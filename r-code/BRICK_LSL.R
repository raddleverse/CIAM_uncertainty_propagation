...
#
# Updates made by Catherine Ledna (March 30, 2019): 
#   Created adder and subtractor functions to deal with lat/lon addition and subtraction 
#   Created a while loop to search for nearest neighbor fingerprint (lines 141-150)
# =======================================================================================
#
...
)
{
  ...
  filename.fingerprints = "../../data/brick/FINAL_FINGERPRINTS.nc"
  
  ncdata <- nc_open(filename.fingerprints)
  lat = ncvar_get(ncdata, 'lat')
  lon = ncvar_get(ncdata, 'lon')
  fp.gsic = ncvar_get(ncdata, 'GLACIERS')
  ...
  # Change CL 3/29/19 changed to while loop and changed lat/lon adding conditions
  inc <- 1
  while((is.na(fp.ais.loc) | is.na(fp.gsic.loc) | is.na(fp.gis.loc) | is.na(fp.te.loc)) && inc <5){
    
    fp.ais.loc = mean(fp.ais[min(lon_subtractor(ilon,inc)):max(lon_adder(ilon,inc)),min(lat_subtractor(ilat,inc)):max(lat_adder(ilat,inc))], na.rm=TRUE)
    fp.gsic.loc = mean(fp.gsic[min(lon_subtractor(ilon,inc)):max(lon_adder(ilon,inc)),min(lat_subtractor(ilat,inc)):max(lat_adder(ilat,inc))], na.rm=TRUE)
    fp.gis.loc = mean(fp.gis[min(lon_subtractor(ilon,inc)):max(lon_adder(ilon,inc)),min(lat_subtractor(ilat,inc)):max(lat_adder(ilat,inc))], na.rm=TRUE)
    fp.te.loc = 1.0
    fp.lws.loc = 1.0
...
# Functions for operations on lon/lat to avoid hitting boundary issues 
adder <- function(max){
  y <- function(point,n){
    if ((point+n) > max) return(point+n-max) else return(point+n)
  }
}

subtractor <- function(min,max){
  y <- function(point,n){
    if ((point-n)< min) return(point-n+max) else return(point-n)
  }
}
